...
def predict_image(image, model):
    # No need to convert image to array since it's already a tensor
    image_array = tf.keras.preprocessing.image.img_to_array(image)  # Convert tensor to array
    ...
    confidence = round(100*(np.max(predictions[0])), 2)  # Get prediction confidence
...
for images, labels in test_dataset.take(1):
    for i in range(9):
        ...
        # Pass the image tensor directly without converting it to a NumPy array
        predicted_class, confidence = predict_image(images[i], model)  # Pass tensor directly
